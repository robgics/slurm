				      ...
				      uint16_t flags, bool locked)
{
	...
	for (i = 0; i < g_tres_count; i++) {
		...
		printf("***** robg TRES Type: %s", tres_type);
		printf("***** robg   TRES Value: %.2f", tres_value);
		printf("***** robg   TRES Weight: %.2f", tres_weight);
		...
		printf("*****  robg   new bill node: %.2f", to_bill_node);
		printf("*****  robg   new bill global: %.2f", to_bill_global);
	}
